...
async function postAssetPurchase(input) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const transaction = await openTransactionCycle(tx, {
      ...
      cycleReference: buildCycleReference("asset"),
      entrepriseId,
    });

    const journal = await postJournalPair(tx, {
      debitAccount: ppeAccount,
      ...
      administrationId,
      description: `PURCHASE_FIXED_ASSET: ${name.trim()} for ${cost} (${paymentMethod})`,
      entrepriseId,
    });
    ...
    return { transaction, journal, asset: assetRow, product, narrative };
  });
}
...
async function postAssetDisposal(input) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const ppeAccount = await mustFindOrCreateAccount(tx, "1400", "Property Plant and Equipment", "ASSET", "DEBIT", "NON_CURRENT_ASSET", entrepriseId);
    ...
    if (!accumDeprAccount) {
      accumDeprAccount = await tx.Account.create({
        data: {
          Account_Name: "Accumulated Depreciation",
          Account_Type: "ASSET",
          Account_Code_id: accumDeprCodeRow.Account_codes_id,
    ...
    const paymentAccount = proceeds > 0 ? await resolvePaymentAccount(tx, paymentMethod, "receive", entrepriseId) : null;
    ...
    await tx.Records.update({ where: { Records_id: recordsRow.Records_id }, data: { Business_Unit: businessUnit } });

    const transaction = await openTransactionCycle(tx, {
      accountId: paymentAccount ? paymentAccount.Account_id : ppeAccount.Account_id,
      productId: (await tx.Product.findFirst({ where: { Product_Name: asset.Assets_Type, Entreprise_id: entrepriseId } }))?.Product_id
    ...
    if (accumulatedDepreciation > 0) {
      journal.push(
        ...(await postJournalPair(tx, {
          ...
          periodId: openPeriod.Structures_id,
          administrationId,
          description: `DISPOSE_FIXED_ASSET: clear accumulated depreciation on ${asset.Assets_Type}`,
          entrepriseId,
        }))
      );
    }
    if (proceeds > 0) {
      journal.push(
        ...(await postJournalPair(tx, {
          ...
          periodId: openPeriod.Structures_id,
          administrationId,
          description: `DISPOSE_FIXED_ASSET: proceeds from ${asset.Assets_Type} (${paymentMethod})`,
          entrepriseId,
        }))
      );
    }
    journal.push(
      ...(await postJournalPair(tx, {
        ...
        periodId: openPeriod.Structures_id,
        administrationId,
        description: `DISPOSE_FIXED_ASSET: remove ${asset.Assets_Type} at cost`,
        entrepriseId,
      }))
    );
    if (gainLoss !== 0) {
      journal.push(
        ...(await postJournalPair(tx, {
          ...
          periodId: openPeriod.Structures_id,
          administrationId,
          description: `DISPOSE_FIXED_ASSET: ${gainLoss > 0 ? "gain" : "loss"} on disposal of ${asset.Assets_Type}`,
          entrepriseId,
        }))
      );
    }
    ...
    const narrative = await writeNarrative(tx, catalogue, transaction, recordsRow, {
      Product_Name: asset.Assets_Type,
...
async function postDepreciationRun(input) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (!depExpenseAccount) {
      depExpenseAccount = await tx.Account.create({
    ...
    const transaction = await openTransactionCycle(tx, {
      accountId: depExpenseAccount.Account_id,
      productId,
      quantity: 1,
      amount,
      ...
      businessUnit,
      recordsId: recordsRow.Records_id,
      cycleReference: buildCycleReference("depreciation"),
      entrepriseId,
    });

    const journal = await postJournalPair(tx, {
      debitAccount: depExpenseAccount,
      creditAccount: accumDeprAccount,
      amount,
      catalogueId: catalogue.Catalogue_id,
      transactionId: transaction.Transactions_id,
      productId,
      periodId: openPeriod.Structures_id,
      administrationId,
      description: `RECORD_DEPRECIATION: ${asset.Assets_Type}`,
      entrepriseId,
    });
    ...
    return { transaction, journal, amount, newAccumulated, newCarrying, narrative };
  });
}
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const recordsRow = await tx.Records.create({
      data: {
        ...
        Administration_id: administrationId,
        Batch_Status: "OPEN",
        Records_Totals: round2(writeDownAmount),
        Entreprise_id: entrepriseId,
      },
    });
    ...
    const transaction = await openTransactionCycle(tx, {
      accountId: impairAccount.Account_id,
      productId,
      quantity: 1,
      ...
      businessUnit,
      recordsId: recordsRow.Records_id,
      cycleReference: buildCycleReference("impairment"),
      entrepriseId,
    });

    const journal = await postJournalPair(tx, {
      debitAccount: impairAccount,
      creditAccount: ppeAccount,
      amount: round2(writeDownAmount),
      catalogueId: catalogue.Catalogue_id,
      transactionId: transaction.Transactions_id,
      ...
      periodId: openPeriod.Structures_id,
      administrationId,
      description: `RECORD_IMPAIRMENT: ${asset.Assets_Type}${reason ? " — " + reason : ""}`,
      entrepriseId,
    });
    ...
    const narrative = await writeNarrative(tx, catalogue, transaction, recordsRow, {
      Product_Name: asset.Assets_Type,
      Amount: writeDownAmount.toFixed(2),
      Reason: reason,
    }, entrepriseId);

    return { transaction, journal, newCarrying, narrative };
  });
}
...
async function postAssetRevaluation(input) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const ppeAccount = await tx.Account.findFirst({ where: { Account_Code_id: ppeCodeRow.Account_codes_id, Entreprise_id: entrepriseId } });
    if (!ppeAccount) throw new PostingError("PPE account (1400) is not seeded.");
    ...
    if (isIncrease) {
      ...
      movementAccount = await tx.Account.findFirst({ where: { Account_Code_id: surplusCodeRow.Account_codes_id, Entreprise_id: entrepriseId } });
      if (!movementAccount) {
        movementAccount = await tx.Account.create({
      ...
      movementAccount = await tx.Account.findFirst({ where: { Account_Code_id: lossCodeRow.Account_codes_id, Entreprise_id: entrepriseId } });
      if (!movementAccount) {
        movementAccount = await tx.Account.create({
    ...
    const recordsRow = await tx.Records.create({
      data: {
        ...
        Administration_id: administrationId,
        Batch_Status: "OPEN",
        Records_Totals: Math.abs(change),
        Entreprise_id: entrepriseId,
      },
    });
    ...
    const transaction = await openTransactionCycle(tx, {
      accountId: ppeAccount.Account_id,
      productId,
      quantity: 1,
      ...
      businessUnit,
      recordsId: recordsRow.Records_id,
      cycleReference: buildCycleReference("revaluation"),
      entrepriseId,
    });

    const journal = await postJournalPair(tx, {
      debitAccount: isIncrease ? ppeAccount : movementAccount,
      creditAccount: isIncrease ? movementAccount : ppeAccount,
      amount: Math.abs(change),
      catalogueId: catalogue.Catalogue_id,
      transactionId: transaction.Transactions_id,
      productId,
      periodId: openPeriod.Structures_id,
      administrationId,
      description: `${eventName}: ${asset.Assets_Type} ${isIncrease ? "up" : "down"} ${Math.abs(change)} to ${newValue}${reason ? " — " + reason : ""}`,
      entrepriseId,
    });
    ...
    return { transaction, journal, newCarrying: round2(newValue), change, narrative };
  });
}
