/**
 * till.js — the Point-of-Sale domain: ringing up a basket of goods,
 * either sold to a customer or bought into stock.
 */
  ...
  return prisma.$transaction(async (tx) => {
    ...
    for (const line of lines) {
      ...
      if (mode === "sell") {
        ...
        const incomeAccount = isUtility ? accounts["4600"] : isService ? accounts["4400"] : accounts["4000"];
        ...
        //
        // WARNING: money.js's Cash Flow route classifies Journal rows by
        // matching against these exact strings (desc.startsWith("SELL
        // GOODS") etc.) to build the Direct Method breakdown. Changing
        // any label here without updating that classification logic will
        // silently break Direct-vs-Indirect reconciliation — this
        // happened for real once already (the labels were reworded from
        // SELL_GOODS_CASH to "SELL GOODS" but money.js kept checking for
        // the old underscored form, so every cash-touching Journal row
        ...
        if (!isUtility && !isService) {
          ...
          if (accounts["5000"] && cogsAmount > 0) {
            postedJournal.push(
              await postJournalPair(tx, {
                debitAccount: accounts["5000"], // COGS
                creditAccount: accounts["1100"], // Inventory
                amount: cogsAmount,
        ...
        const isUtility = !!product.Is_Utility;
        const isService = !isUtility && !!product.Is_Service;
        const debitAccount = isUtility ? accounts["5400"] : isService ? accounts["5450"] : accounts["1100"];
        ...
        // shown separately at the end of the line.
        // WARNING: see the identical note on sellDescriptionLabel — money.js
        // matches against these exact strings too.
