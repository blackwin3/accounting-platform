...
async function postSuccession(input) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const equityRows = await tx.Equity.findMany({ where: { Entreprise_id: entrepriseId } });
...
// ─── SECTION 5: BUSINESS SUCCESSION ──────────────────────────────────────────
//
// The transfer of ownership from the current owner to a named successor.
// This is a genuine accounting event: the equity structure of the business
// changes — the outgoing owner's capital balance becomes the incoming
// owner's capital balance. IAS 1 requires disclosure of changes in equity;
// this posting captures that change as a real Journal entry rather than an
// off-books narrative.
//
// The succession cycle as implemented:
//   1. Stakeholders and Management rows already have Inheritance_Status
//      (CURRENT_OWNER / SUCCESSOR / ADVISOR / EXCLUDED / RETIRED / DECEASED)
//   2. postSuccession transfers ownership: changes the Management rows'
//      Inheritance_Status, posts the equity transfer, writes a Knowledge
//      entry explaining the decision, and updates Access_Level so the
//      successor can actually operate the business.
//   3. A Succession Report can be generated from the resulting state —
//      the platform's existing Reports, Knowledge, and Narrative tables
//      already hold everything needed.

/**
 * postSuccession — the formal transfer of ownership from one person to
 * another. Posts a genuinely balanced equity transfer entry:
 *   DR Outgoing Owner Capital (reduces outgoing equity)
 *   CR Incoming Owner Capital (creates incoming equity)
 *
 * Updates Management rows: outgoing owner → RETIRED, successor → CURRENT_OWNER
 * with OWNER_FULL access. Writes a Knowledge entry explaining the decision.
 *
 * This is the Tier 3 gap being closed: "no accounting event for succession."
 */
async function postSuccession(input) {
  const { outgoingManagementId, incomingManagementId, reason, transferAmount = null, administrationId = null, entrepriseId } = input;

  if (!entrepriseId) throw new PostingError("entrepriseId is required.");
  if (!outgoingManagementId) throw new PostingError("outgoingManagementId is required — who is handing over.");
  if (!incomingManagementId) throw new PostingError("incomingManagementId is required — who is taking over.");
  if (!reason || !reason.trim()) throw new PostingError("A reason is required — succession must be documented for auditors, family members, and the incoming owner.");
  if (outgoingManagementId === incomingManagementId) throw new PostingError("The outgoing and incoming owner must be different people.");

  return prisma.$transaction(async (tx) => {
    const outgoing = await tx.Management.findUnique({ where: { Administration_id: Number(outgoingManagementId) } });
    if (!outgoing || outgoing.Entreprise_id !== entrepriseId) throw new PostingError("Outgoing owner not found for this business.");
    if (outgoing.Inheritance_Status !== "CURRENT_OWNER") throw new PostingError(`The outgoing person's Inheritance_Status is "${outgoing.Inheritance_Status}", not CURRENT_OWNER — only a current owner can hand over.`);

    const incoming = await tx.Management.findUnique({ where: { Administration_id: Number(incomingManagementId) } });
    if (!incoming || incoming.Entreprise_id !== entrepriseId) throw new PostingError("Incoming successor not found for this business.");
    ...
    // Compute the transfer amount — the outgoing owner's equity balance.
    // If a specific transferAmount is given (e.g. a negotiated buyout price
    // different from book equity), use that; otherwise use the full
    // equity balance computed from the Equity table.
    const equityRows = await tx.Equity.findMany({ where: { Entreprise_id: entrepriseId } });
    const totalEquity = round2(equityRows.reduce((sum, e) => sum + Number(e.Net_Amount || 0), 0));
    const amount = transferAmount != null ? round2(Number(transferAmount)) : totalEquity;

    if (amount < 0) throw new PostingError("Transfer amount cannot be negative.");

    let journal = [];

    if (amount > 0) {
      // Post the equity transfer: DR Outgoing Owner Capital, CR Incoming Owner Capital
      const eventName = "SUCCESSION_TRANSFER";
      let catalogue = await tx.Catalogue.findFirst({ where: { Event_Name: eventName, Entreprise_id: entrepriseId } });
      if (!catalogue) {
        catalogue = await tx.Catalogue.create({
          data: {
            Event_Name: eventName,
            Event_Description: "Transfer of ownership equity from outgoing owner to successor. DR Outgoing Owner Capital (3100) CR Incoming Owner Capital (3100). Equity reallocation — no cash movement, no external party. IAS 1 Change in Equity.",
            Debit_Account_code: "3100",
            Credit_Account_code: "3100",
            Posting_Complexity: "SIMPLE",
            Cash_Flow_Category: "NONE",
            ...
            Documentation_type: "NONE",
            Report_trigger: "EQUITY_STATEMENT",
            Escalation_Role: "LAWYER",
            Cycle_type: "INHERITANCE",
            Alert_Required: 1,
            Narrative_template: "Business succession: {OutgoingName} transfers KES {Amount} to {IncomingName}.",
            Evidence_template: "NONE",
            Report_sections: "EQUITY_STATEMENT:ChangesInEquity|SUCCESSION_REPORT:OwnershipTransfer",
            Default_Business_Unit: "SHOP",
            Is_Active: 1,
            Version_No: 1,
            Effective_From: new Date("2020-04-01"),
      ...
      const equityAccount = await tx.Account.findFirst({
        where: { Account_Type: "EQUITY", Entreprise_id: entrepriseId },
      });
      if (!equityAccount) throw new PostingError("No equity account found — capital must have been injected before succession.");

      const product = await findOrCreateExpensePlaceholder(tx, "Succession Transfer", entrepriseId);

      const recordsRow = await tx.Records.create({
        data: { Catalogue_id: catalogue.Catalogue_id, Records_type: "TRANSACTION_BATCH", Records_date: new Date(), Period_id: openPeriod.Structures_id, Business_Unit: "SHOP", Administration_id: administrationId, Batch_Status: "OPEN", Records_Totals: amount, Entreprise_id: entrepriseId },
      });

      const transaction = await openTransactionCycle(tx, {
        accountId: equityAccount.Account_id, productId: product.Product_id, quantity: 1, amount, businessEvent: "INHERITANCE", cycleType: "INHERITANCE", businessUnit: "SHOP", recordsId: recordsRow.Records_id, cycleReference: buildCycleReference("succession"), entrepriseId,
      });
      ...
      journal = await postJournalPair(tx, {
        ...
        periodId: openPeriod.Structures_id,
        administrationId,
        description: `SUCCESSION_TRANSFER: ${outgoing.Management_Name || "outgoing"} → ${incoming.Management_Name || "incoming"} — KES ${amount}`,
        entrepriseId,
      });

      // Record the equity shift on both sides
      await tx.Equity.create({
        data: { Catalogue_id: catalogue.Catalogue_id, Account_id: equityAccount.Account_id, Records_id: recordsRow.Records_id, Equity_type: "Owner Capital Withdrawal", Net_Amount: round2(-amount), Period: new Date(), Entreprise_id: entrepriseId },
      });
      await tx.Equity.create({
        data: { Catalogue_id: catalogue.Catalogue_id, Account_id: equityAccount.Account_id, Records_id: recordsRow.Records_id, Equity_type: "Owner Capital", Net_Amount: round2(amount), Period: new Date(), Entreprise_id: entrepriseId },
      });
    }

    // Update Management rows
    await tx.Management.update({
      where: { Administration_id: outgoing.Administration_id },
      data: { Inheritance_Status: "RETIRED", Access_Level: "VIEWER" },
    });
    await tx.Management.update({
      where: { Administration_id: incoming.Administration_id },
      data: { Inheritance_Status: "CURRENT_OWNER", Access_Level: "OWNER_FULL" },
    });

    // Write the Knowledge entry — this is the institutional memory of WHY
    // the succession happened, not just THAT it happened.
    await tx.Knowledge.create({
      data: {
        Knowledge_type: "DECISION_REASON",
        Explanation: `Business succession: ${outgoing.Management_Name || "outgoing owner"} (Management #${outgoing.Administration_id}) handed over to ${incoming.Management_Name || "incoming owner"} (Management #${incoming.Administration_id}). Transfer amount: KES ${amount}.`,
        Decision_Reason: reason.trim(),
        Context: "SUCCESSION",
        Confidence_Level: 5,
        Language: "en",
        Audience: incoming.Administration_id,
        Author: outgoing.Stakeholder_id,
        Entry_date: new Date(),
        Entreprise_id: entrepriseId,
      },
    });
    ...
    return {
      ...
      outgoingName: outgoing.Management_Name,
      incomingName: incoming.Management_Name,
    };
  });
}
