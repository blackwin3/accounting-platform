...
async function getCapitalPosition({ entrepriseId }) {
  ...
  // No $transaction wrapper — this is read-only and the transaction
  // timeout on Render's free tier causes hangs. Each query runs
  // independently which is fine for a reporting function.
  const tx = prisma;
  ...
  // Owner Capital — query Equity via Account_id since Entreprise_id
  // may not exist on the Equity table yet.
  ...
  let rentalAssets = [];
  try {
    rentalAssets = await tx.Assets.findMany({ where: { Is_Rental_Property: 1, Entreprise_id: entrepriseId } });
  } catch { /* Is_Rental_Property may not exist */ }
  ...
  return {
    ...
    rentalPropertyCount: rentalAssets.length,
    rentalPropertiesWithTenant: rentalAssets.filter((a) => a.Tenant_Stakeholder_id).length,
    totalCapitalDeployed: round2(cashOnHand + investmentsHeld + rentalPropertyCarryingValue),
  };
}
