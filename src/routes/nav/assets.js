...
const { prisma, getAssetsSummary, getDisposalHistory, getDepreciationScheduleOverview } = require("../../services/postingEngine");
...
router.get("/assets", async (req, res) => {
  try {
    ...
    // Whole-business totals and disposal history, genuinely computed —
    // matches capital_management.js's pattern on the Funds page, a
    // read-only summary layer over assets.js, not a replacement.
    const [assetsSummary, disposalHistory, depreciationSchedule] = await Promise.all([
      getAssetsSummary({ entrepriseId }),
      getDisposalHistory({ entrepriseId, limit: 10 }),
      getDepreciationScheduleOverview({ entrepriseId }),
    ]);
