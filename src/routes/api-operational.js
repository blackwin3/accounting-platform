...
// POST /api/livestock/register { productId, tag, category, sex, birthDate, fairValue, condition, growthStage, parentResourcesId }
router.post("/livestock/register", async (req, res) => {
  try {
    const { productId, tag, category, sex, birthDate, fairValue, condition, growthStage, parentResourcesId } = req.body;
    const animal = await registerAnimal({
      ...
      parentResourcesId: parentResourcesId || null,
      entrepriseId: req.currentUser.Entreprise_id,
    });
    res.json({ ok: true, resourcesId: animal.Resources_id });
  } catch (err) {
    if (err instanceof PostingError) return res.status(400).json({ error: err.message });
    console.error(err);
    res.status(500).json({ error: "Internal error registering animal or planting" });
  }
});

// POST /api/livestock/:id/review { condition, fairValue, growthStage, note }
router.post("/livestock/:id/review", async (req, res) => {
  try {
    const { condition, fairValue, growthStage, note } = req.body;
...
// POST /api/livestock/:id/loss { reason, lossType }
router.post("/livestock/:id/loss", async (req, res) => {
  try {
    const { reason, lossType } = req.body;
    const result = await recordAnimalLoss({
      resourcesId: Number(req.params.id),
      reason,
      lossType: lossType || "DEATH_OR_SPOILAGE",
      businessUnit: req.currentBusinessUnit,
      administrationId: req.currentUser.Administration_id,
      entrepriseId: req.currentUser.Entreprise_id,
    });
    res.json({ ok: true, lossValue: result.lossValue });
  } catch (err) {
    if (err instanceof PostingError) return res.status(400).json({ error: err.message });
    console.error(err);
...
// POST /api/livestock/:id/birth { tag, sex, fairValue, condition, growthStage }
// :id is the mother's own Resources_id
router.post("/livestock/:id/birth", async (req, res) => {
  try {
    const { tag, sex, fairValue, condition, growthStage } = req.body;
    const result = await recordBirth({
      motherResourcesId: Number(req.params.id),
      tag,
      sex,
      fairValue: fairValue != null ? Number(fairValue) : null,
      condition,
      growthStage,
      businessUnit: req.currentBusinessUnit,
      administrationId: req.currentUser.Administration_id,
    ...
    res.json({ ok: true, resourcesId: result.animal.Resources_id, gainValue: result.gainValue });
  } catch (err) {
    if (err instanceof PostingError) return res.status(400).json({ error: err.message });
    console.error(err);
...
// POST /api/livestock/:id/harvest { outputProductId, outputQuantity, harvestValue }
// :id is the planting's own Resources_id
router.post("/livestock/:id/harvest", async (req, res) => {
  try {
    const { outputProductId, outputQuantity, harvestValue } = req.body;
    const result = await recordHarvest({
      plantingResourcesId: Number(req.params.id),
      outputProductId: Number(outputProductId),
      outputQuantity: Number(outputQuantity),
      harvestValue: harvestValue != null ? Number(harvestValue) : null,
      businessUnit: req.currentBusinessUnit,
      administrationId: req.currentUser.Administration_id,
      entrepriseId: req.currentUser.Entreprise_id,
    });
    res.json({ ok: true, outputQuantity: result.outputQuantity, finalValue: result.finalValue });
  } catch (err) {
    if (err instanceof PostingError) return res.status(400).json({ error: err.message });
    console.error(err);
...
// POST /api/livestock/bulk-planting { productId, fieldId, plotCount, plantingDate, fairValuePerPlot, growthStage, note }
// Registers multiple crop plantings in a single batch — one Resources row
// per plot with auto-generated sequential tags from the fieldId prefix.
router.post("/livestock/bulk-planting", async (req, res) => {
  try {
    const { productId, fieldId, plotCount, plantingDate, fairValuePerPlot, growthStage, note } = req.body;
    const result = await bulkPlanting({
      productId: Number(productId),
      fieldId,
      plotCount: Number(plotCount),
      plantingDate,
      fairValuePerPlot: fairValuePerPlot != null ? Number(fairValuePerPlot) : 0,
      growthStage,
      note,
      administrationId: req.currentUser.Administration_id,
      entrepriseId: req.currentUser.Entreprise_id,
    });
    res.json({ ok: true, plotsRegistered: result.plotsRegistered, fieldId: result.fieldId, totalFairValue: result.totalFairValue });
  } catch (err) {
    if (err instanceof PostingError) return res.status(400).json({ error: err.message });
    console.error(err);
    res.status(500).json({ error: "Internal error registering bulk planting" });
  }
});

// POST /api/seasonal-labour { stakeholderId, description, days, dailyRate, paymentMethod, labourType }
// Records payment to temporary/seasonal workers — farm hands during planting,
// extra security during harvest, casual repair labour. Posts DR Casual Labour
// Expense (5250) CR Cash/Mobile/Bank.
router.post("/seasonal-labour", async (req, res) => {
  try {
    const { stakeholderId, description, days, dailyRate, paymentMethod, labourType } = req.body;
    const result = await postSeasonalLabour({
      stakeholderId: stakeholderId ? Number(stakeholderId) : null,
      description,
      days: Number(days),
    ...
    res.json({ ok: true, transactionId: result.transaction.Transactions_id, amount: result.amount, labourType: result.labourType });
  } catch (err) {
    if (err instanceof PostingError) return res.status(400).json({ error: err.message });
    console.error(err);
...
// POST /api/services/start { productId, hourlyRate, client }
router.post("/services/start", async (req, res) => {
  try {
    const { productId, hourlyRate, client } = req.body;
    const engagement = await startServiceEngagement({
      productId: Number(productId),
      hourlyRate: Number(hourlyRate),
      client,
      entrepriseId: req.currentUser.Entreprise_id,
    });
    res.json({ ok: true, resourcesId: engagement.Resources_id });
  } catch (err) {
    if (err instanceof PostingError) return res.status(400).json({ error: err.message });
    console.error(err);
...
// POST /api/services/:id/bill { paymentMethod, finalAmount }
router.post("/services/:id/bill", async (req, res) => {
  try {
    ...
    res.json({ ok: true, billedAmount: result.billedAmount, hoursLogged: result.hoursLogged });
  } catch (err) {
    if (err instanceof PostingError) return res.status(400).json({ error: err.message });
    console.error(err);
...
// POST /api/lessor/lease-out { resourcesId, stakeholderId, amount, paymentMethod, notes }
router.post("/lessor/lease-out", async (req, res) => {
  try {
    const { resourcesId, stakeholderId, amount, paymentMethod, notes } = req.body;
    const result = await leaseOutInventory({
      resourcesId: Number(resourcesId),
...
// POST /api/lessor/:id/return { condition }
router.post("/lessor/:id/return", async (req, res) => {
  try {
    const { condition } = req.body;
    await returnLeasedInventory({
      resourcesId: Number(req.params.id),
    ...
    if (err instanceof PostingError) return res.status(400).json({ error: err.message });
    console.error(err);
    res.status(500).json({ error: "Internal error returning leased unit" });
  }
});

// POST /api/lessor/equipment/hire-out { assetsId, stakeholderId, amount, paymentMethod }
router.post("/lessor/equipment/hire-out", async (req, res) => {
  try {
    const { assetsId, stakeholderId, amount, paymentMethod } = req.body;
    await hireOutEquipment({
...
// POST /api/lessor/equipment/:id/end-hire
router.post("/lessor/equipment/:id/end-hire", async (req, res) => {
  try {
    await endEquipmentHire({ assetsId: Number(req.params.id), entrepriseId: req.currentUser.Entreprise_id });
    res.json({ ok: true });
  } catch (err) {
    if (err instanceof PostingError) return res.status(400).json({ error: err.message });
    console.error(err);
